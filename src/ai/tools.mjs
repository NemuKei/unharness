...
export const AI_TOOLS = Object.freeze([
  ...
  tool('propose_change', 'propose', '利用者の承認が必要。提案の作成は設定を変えない。Create a local proposal for user review.', { kind: z.enum(['initial', 'add', 'remove', 'restore']), mode: z.enum(['normal', 'unseal', 'trueform']), setupReviewId: id.optional(), items: z.array(z.strictObject({ sourceId, reason: text(200) })).max(32) }, true),
  tool('read_proposals', 'proposals', 'Read up to 20 recent local proposals and their current states.'),
  tool('decide_proposal', 'decide-proposal', 'Approve or dismiss one proposal after the user decides. Approval applies its guarded setup and mode plan; it requires user approval.', { proposalId: id, decision: z.enum(['approve', 'dismiss']) }, true, true),
  tool('review_replaced_source', 'review-replaced-source', 'Review one registered Skill whose folder was replaced by another application. Reads the new body and its current settings as data, records a private review, and changes no source files.', { sourceId }, true),
  tool('apply_replaced_source', 'apply-replaced-source', 'After the user confirms the new folder and any changed Skill body, adopt one guarded successor registration. Leaves old Normal, favorites and evidence intact; the previous release mode must be prepared again.', { reviewId: id, confirmedNewLocation: z.literal(true), confirmedChangedContent: z.literal(true).optional() }, true, true),
  ...
  tool('review_authored_appearance', 'review-authored-appearance', 'Review explicit PNG parts from an issued creation place. Accepts known part IDs, never paths. Retains that place\'s exact base version and preserves original files. Creates a review; show its composition before save_appearance_import.', { authoringId: id, importId: uuid, expectedStateId: id.nullable(), name: text(80),
    author: z.string().max(80).regex(/^[^\u0000-\u001f\u007f-\u009f]*$/), partIds: z.array(z.enum([...getAppearanceTemplate().parts.map(p => p.id),...Object.keys(ENTITY_SHEETS)])).min(1).max(13) }, true),
