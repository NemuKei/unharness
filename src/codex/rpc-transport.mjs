...
// Internal transport; each factory supplies its own immutable method policy.
export function createRpcTransport({
  ...
  function assertMethod(method) {
    if (!methods.has(method)) throw localError('forbidden-method');
  }
  let child;
  try {
    ...
    return failedClient(error, assertMethod);
  }
...
function failedClient(error, assertMethod) {
  return {
    request(method) {
      try {
        assertMethod(method);
