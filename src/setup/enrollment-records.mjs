...
export function validateAdditions(value) {
  ...
  for (const a of value) {
    exactKeys(a, ['sourceId', 'origin', 'reason', 'unseal', 'trueform'], [], 'enrollment-proposal-invalid');
    if (!/^skill-[a-f0-9]{64}$/.test(a.sourceId) || !['self', 'external'].includes(a.origin) ||
        !['automatic', 'manual'].includes(a.unseal) || !['automatic', 'manual'].includes(a.trueform) ||
...
export async function loadEnrollmentReview(w, reviewId) {
  try {
    ...
    if (p.role !== 'enrollment-review' || p.schemaVersion !== 1 || !Number.isSafeInteger(p.revision) || p.revision < 0 ||
        ['scopeId', 'normalId', 'beforeId', 'previousSetupId', 'discoveryId', 'nextScopeId', 'nextNormalId', 'nextSnapshotId', 'setupReviewId', 'setupId'].some(k => !hash(p[k]))) fail('enrollment-record-invalid');
    validateAdditions(p.additions);
...
export const enrollmentSummary = p => ({ reviewId: p.reviewId, scopeId: p.scopeId, nextScopeId: p.nextScopeId,
