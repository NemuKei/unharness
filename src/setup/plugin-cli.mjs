...
export const PLUGIN_USAGE = `  node bin/unharness.mjs plugin <status|mcp|recovery> --data-directory <native-plugin-data>
...
function parse(argv) {
  ...
  if (!['status', 'configure', 'mcp', 'recovery'].includes(command)) return null;
...
export async function pluginMain(argv, { stdin = process.stdin, stdout = process.stdout, stderr = process.stderr } = {}) {
  ...
  try {
    ...
    if (options.command === 'configure') {
      await configurePlugin({ dataDirectory: options.dataDirectory, ...options.input });
      recovery = await (await import('./plugin-recovery.mjs')).prepareInstalledRecovery(options.dataDirectory);
    }
    const binding = await openPluginBinding({ dataDirectory: options.dataDirectory });
    if (options.command === 'recovery') {
      const { preparePluginRecovery } = await import('./plugin-recovery.mjs');
      stdout.write(JSON.stringify(await preparePluginRecovery({ dataDirectory: options.dataDirectory,
        distributionRoot: fileURLToPath(new URL('../../', import.meta.url)).replace(/\/$/, '') })) + '\n');
      return 0;
    }
