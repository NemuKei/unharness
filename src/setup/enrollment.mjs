...
import { openWorkspace, activeNormalId, loadNormal, loadSnapshot, saveSnapshot, record, registeredSkill,
  loadScopeLineage, writeJson, unlink, newPreparation } from '../sources/records.mjs';
...
async function current(w) {
  ...
  if (reopened.scopeId !== w.scopeId || !equal(reopened.reg, w.reg) || !equal(reopened.state, w.state)) fail('stale-plan');
...
export async function reviewEnrollment(args) {
  ...
  const additions = validateAdditions(args.additions);
  return locked(args.workspace, async w => {
    ...
    const setupReviewId = await record(w.workspace, 'input', { role: 'setup-review', schemaVersion: 1,
    ...
    const setupId = await record(w.workspace, 'application', { role: 'release-setup', schemaVersion: 1, scopeId: nextScopeId, reviewId: setupReviewId });
    ...
    const reviewId = await record(w.workspace, 'input', { role: 'enrollment-review', schemaVersion: 1, scopeId: w.scopeId,
...
export async function applyEnrollment(args) {
  ...
  return locked(args.workspace, async w => {
    ...
    // Re-run the current forward control boundary against the frozen proposal.
    ...
    await writeJson(join(w.workspace, 'pending.json'), { kind: 'unharness-user-source-enrollment-pending',
      scopeId: w.scopeId, reviewId: p.reviewId, beforeState: w.state, afterState }, true);
