...
import { PUBLIC_WEB_ORIGIN, CONNECTION_OPERATIONS as REMOTE_OPERATIONS } from '../web/src/connection-contract.ts';
...
const pending = () => ({ pairingId: randomUUID(), status: 'approved', ticket: 'b'.repeat(64), protocolVersion: 2,
  launchId: view.metadata.launchId, webOrigin: PUBLIC_WEB_ORIGIN, expiresAt: Date.now() + 60000,
  target: { application: 'codex', scopeId: 'a'.repeat(64), collectionScopeId: 'a'.repeat(64) }, operations: [...REMOTE_OPERATIONS] });
...
test('local handoffs accept only one exact pairing ID and public links contain only the short-lived ticket', () => {
  ...
  assert.deepEqual(Object.fromEntries(new URLSearchParams(link.hash.slice(1))), {
    unharness: '2', port: '45678', launch: value.launchId, ticket: value.ticket,
  });
...
test('local approval refuses changed targets, broader permissions, and malformed connection details', () => {
  for (const mutate of [v => { v.webOrigin += '.other.test'; }, v => { v.target.scopeId = 'c'.repeat(64); },
    v => { v.target.collectionScopeId = 'c'.repeat(64); }, v => { delete v.target.collectionScopeId; },
    v => { v.launchId = randomUUID(); }, v => { v.protocolVersion = 1; }, v => { v.operations.push('approve'); },
...
test('local approval binds an enrolled scope to its preserved root collection', () => {
  const enrolled = structuredClone(view);
  enrolled.source.registration.rootScopeId = 'd'.repeat(64);
  const value = pending();
  assert.throws(() => readLocalPairing(value, enrolled, value.pairingId));
  value.target.collectionScopeId = enrolled.source.registration.rootScopeId;
  assert.equal(readLocalPairing(value, enrolled, value.pairingId).target.collectionScopeId, 'd'.repeat(64));
});
