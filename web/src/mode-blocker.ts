...
export function modeBlocker(state: {
  ...
  if (state.conflict && !state.modePlanningAvailable) return { kind: 'changes', message: t("保存時と今の設定が異なるため、切替を止めています。", "Switching is paused because current settings differ from the saved version.") };
