...
export function setupHandoffPrompt(view: SourceView, route: SetupRoute): string {
  ...
  const lines = [
    ...
    t("新しい提案はschemaVersion=3を使い、read_setupにschemaVersion:3を渡して取得したinventoryIdを指定してください。零式は追加指示をなくし、登録した通常Skillをそれぞれ無効か手動にします。限定解除は零式を継承し、選んだSkillだけ手動・自動へ上げます。Normalで無効だったSkillを有効にする場合も、その変更を明示して確認してください。", "Use schemaVersion=3 for new proposals and the inventoryId returned by read_setup with schemaVersion:3. TRUEFORM removes optional instructions and sets each registered ordinary Skill to disabled or manual. UNSEAL inherits TRUEFORM and raises only selected Skills to manual or automatic. Explicitly review any change that enables a Skill disabled in Normal."),
    ...
    t("切替対象の追加指示はグローバルAGENTS.mdとして読み込む任意の指示です。固定の最小ガイドか、なしを相談します。元のAGENTS.mdとリポジトリのAGENTS.mdを保持し、既存のAGENTS.override.mdの切替処理を使ってください。同じCodex設定を使う他のプロジェクトの新しいタスクにも関係することを示してください。零式の選択を変えるときは両モードへの影響を一緒に示してください。公式プラグインのSkill・MCP・hook等は保持し、実行状態の未確認を示してください。旧規則の保存版・お気に入りは書き換えないでください。", "Optional instruction targets are loaded through global AGENTS.md. Discuss the fixed minimal guide or no guide. Preserve the original AGENTS.md and repository AGENTS.md and use the existing AGENTS.override.md switching operation. Explain its effect on new tasks in other projects sharing this Codex profile. Show the impact on both modes when TRUEFORM choices change. Retain official-plugin Skills, MCP and hooks and state that their runtime is unconfirmed. Never rewrite saved versions or favorites under earlier rules."),
