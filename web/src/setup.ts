...
export function setupHandoffPrompt(view: SourceView, route: SetupRoute): string {
  ...
  const lines = [
    ...
    '初回登録がまだなら、このMacの確認画面で切替対象と通常の構成の保存を案内してください。登録や許可の確認を省略せず、相談や依頼文のコピーだけで初期設定済みと扱わないでください。',
    ...
    '切替対象の追加指示はグローバルAGENTS.mdとして読み込む任意の指示です。固定の最小ガイドか、なしを相談します。元のAGENTS.mdとリポジトリのAGENTS.mdを保持し、既存のAGENTS.override.mdの切替処理を使ってください。同じCodex設定を使う他のプロジェクトの新しいタスクにも関係することを示してください。零式の選択を変えるときは両モードへの影響を一緒に示してください。公式プラグインのSkill・MCP・hook等は保持し、実行状態の未確認を示してください。旧規則の保存版・お気に入りは書き換えないでください。',
