...
export const CONNECTION_OPERATIONS = ["status", "plan", "apply", "operation-status"] as const;
...
export type ConnectionBinding = {
  protocolVersion: 1; launchId: string; webOrigin: string; expiresAt: number;
  target: { application: "codex" | "claude"; scopeId: string }; operations: string[];
};
...
export function readConnectionBinding(value: unknown): ConnectionBinding {
  ...
  connectionFields(target, ["application", "scopeId"]);
  if (v.protocolVersion !== 1 || !isConnectionId(v.launchId) || v.webOrigin !== PUBLIC_WEB_ORIGIN
    || !Number.isSafeInteger(v.expiresAt) || (v.expiresAt as number) < 0 || !isConnectionHash(target.scopeId)
  ...
  return { protocolVersion: 1, launchId: v.launchId, webOrigin: PUBLIC_WEB_ORIGIN, expiresAt: v.expiresAt as number,
    target: { application: target.application as "codex" | "claude", scopeId: target.scopeId }, operations: [...CONNECTION_OPERATIONS] };
}
