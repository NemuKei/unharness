...
export function ReplacedSourceReviewSheet({ review, busy, uncertain = false, confirmedLocation, confirmedContent,
  onLocation, onContent, onApply, onClose }: { review: ReplacedSourceReview; busy: boolean; uncertain?: boolean;
  ...
  return <section className="replaced-source-sheet" aria-label={t('新しいSkillを確認', 'Review the new Skill')}>
    ...
    {review.missingPreparedFiles.length > 0 && <p>{t('前に置いた零式の設定が見つかりません。登録後に零式を準備し直してください。',
      'Earlier TRUEFORM settings are missing. Prepare TRUEFORM again after registration.')}</p>}
...
export function ReplacedSourcePanel({ controller: c, sourceId, label }: { controller: Controller; sourceId: string; label: string }) {
  ...
  return <section className="replaced-source-panel" aria-label={t('更新されたSkill', 'Updated Skill')}>
    ...
    {review && <ReplacedSourceReviewSheet review={review} busy={c.busy} uncertain={uncertain} confirmedLocation={confirmedLocation}
