...
const phase = (value: number, from: number, to: number) => ease((value - from) / (to - from));
const mix = (a: number, b: number, t: number) => a + (b - a) * t;
const radians = (degrees: number) => degrees * Math.PI / 180;
const lens = 900;

/** Fold around the physical outer seam; edge lengths remain unchanged in 3D. */
function fold(point: Point2, a: Point2, b: Point2, angle: number): Point3 {
...
function panelAt(panel: SourcePanel, release: number) {
...
export type Cel = ReturnType<typeof celAt>;
...
export function selectCel(cels: readonly Cel[], release: number) {
