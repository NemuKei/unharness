...
import { publicModes } from './public-modes';
import type { SourceMode } from '../sources';
import { siteConfig } from '../site-config';
import { copy } from './guided-copy';
import type { Locale, Scenario, Stage } from './guided-copy';
...
function requestText(kind: RequestKind, locale: Locale, mode: SourceMode) {
  ...
  const ja = {
    ...
    fresh: 'アンハーネスで' + publicModes[mode].title + 'を試したいです。まず実際の状態を確認してください。このデモでは実際の設定を準備していません。必要な準備と、新しいタスクで使う手順を案内してください。',
  };
  const en = {
    ...
    fresh: 'Help me try ' + publicModes[mode].title + ' in Unharness. Check my real state first: the sample demo did not prepare any real settings. Guide the required preparation and how to use a fresh task. Please guide me in English.',
  };
  return (locale === 'ja' ? ja : en)[kind];
}

export function GuidedExperience({ embedded = false, language }: { embedded?: boolean; language?: Locale }) {
  const [conceptLocale, setLocale] = useState<Locale>(initialLocale);
  const locale = language ?? conceptLocale;
  const Main = embedded ? 'section' : 'main';
  const [page, setPage] = useState<Page>(embedded ? 'guide' : 'home');
  ...
  return <div className={"gp-app" + (embedded ? " gp-embedded" : "")} data-page={page} data-stage={stage}>
    {!embedded && <><a href="#gp-main" className="gp-skip">{t.skip}</a>
    ...
    {embedded && <p className="gp-sample-boundary">{locale === "ja" ? "AIと進めるデモ ／ すべてサンプルの設定です。あなたの設定は変わりません。" : "AI-guided demo / All settings are synthetic. Your settings stay unchanged."}</p>}
    ...
    {!embedded && <footer className="gp-footer"><span>{t.footer}</span><div>
