...
test("conflict, stale and unready responses invalidate a known preparation across plan and mutation paths", async (t) => {
  const { Api } = await import("../src/api.ts");
  const { submitOperation, operationFailure, preparationUsability } =
    await import("../src/operations.ts");
  const checkpointId = "d".repeat(64);
  let kind = "fixture-conflict";
  let posts = 0;
  const previous = {
    current: { case: "baseline" },
    conflict: null,
    application: { applicationId: "a".repeat(64) },
    applicationCurrent: true,
    observation: { fixtureMarkerCheck: "matched-record" },
  };
  const server = createServer((req, res) => {
    if (req.url === "/api/bootstrap")
      return res.end(JSON.stringify({ token: "test-token" }));
    posts++;
    res.statusCode = 409;
    res.end(JSON.stringify({ error: { kind, checkpointId } }));
  });
  await new Promise((resolve) => server.listen(0, "127.0.0.1", resolve));
  t.after(() => server.close());
  const api = new Api(`http://127.0.0.1:${server.address().port}`);
  ...
  for (kind of [
    ...
    for (const route of ["/apply", "/observe", "/plan"]) {
      const result =
        route === "/plan"
          ? await api.post(route, {}).catch(operationFailure)
          : await submitOperation(api, route, {});
      assert.equal(
        result.connection,
        "connected",
        "preparation freshness is separate from HTTP connectivity",
      );
      assert.equal(result.invalidatePreparation, true);
      assert.equal(result.checkpointId, checkpointId);
      const view = preparationUsability(
        previous,
        true,
        !result.invalidatePreparation,
      );
      assert.equal(view.canUsePreparation, false);
      assert.equal(
        view.applicationCurrent,
        false,
        "previous matched record is now historical",
      );
      assert.equal(
        previous.applicationCurrent,
        true,
        "retain immutable prior receipt",
      );
      if (["fixture-conflict", "loadout-readback-conflict"].includes(kind))
        assert.equal(
          result.status,
          "uncertain",
          "409 may follow a source write",
        );
    }
  }
  kind = "current-session-unavailable";
  const ordinary = await submitOperation(api, "/observe", {});
  assert.equal(ordinary.invalidatePreparation, false);
  assert.equal(
    preparationUsability(previous, true, true).canUsePreparation,
    true,
  );
  assert.equal(posts, 22, "each deliberate request was sent once");
});

test("a real fixture conflict after its first write cannot leave cached preparation usable", async (t) => {
  const { Api } = await import("../src/api.ts");
  const { submitOperation, preparationUsability } = await import(
    "../src/operations.ts"
  );
  const { createDesktopFixture, changeDesktopFixture, readDesktopFixture } =
    await import("../../src/codex/desktop-fixture.mjs");
  const { mkdtemp, realpath, writeFile, rm } = await import("node:fs/promises");
  const { join } = await import("node:path");
  const { tmpdir } = await import("node:os");
  const parent = await realpath(
    await mkdtemp(join(tmpdir(), "unharness-client-conflict-")),
  );
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      res.end(
        JSON.stringify({
          error: { kind: error.kind, checkpointId: "e".repeat(64) },
        }),
      );
    }
  });
  ...
  const api = new Api(`http://127.0.0.1:${server.address().port}`);
  await api.connect();
  const outcome = await submitOperation(api, "/apply", {});
  assert.equal(writes, 1);
  assert.ok((await readDesktopFixture(fixture.fixture)).state.pending);
  assert.equal(outcome.status, "uncertain");
  assert.equal(outcome.connection, "connected");
  assert.equal(outcome.invalidatePreparation, true);
  assert.equal(outcome.checkpointId, "e".repeat(64));
  const known = {
    current: { case: "baseline" },
    conflict: null,
    applicationCurrent: true,
  ...
  assert.deepEqual(
    preparationUsability(known, true, !outcome.invalidatePreparation),
    { canUsePreparation: false, applicationCurrent: false },
  );
  assert.equal(posts, 1);
});
