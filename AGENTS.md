...
- The 2026-09-10 mode target uses only explicitly selected, verified official-marketplace plugins as optional automatic Skills in TRUEFORM. UNSEAL inherits all of that set and adds reviewed external/self-authored Skills. Official listing is provenance, not provider authorship or a performance guarantee. Required Unharness controls remain a separate exception even before official listing. This target is not implemented by the current v1 preset code.
