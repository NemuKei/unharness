...
- Keep macOS and Windows as design targets, with the first Mac product completed for Codex Desktop. The maintainer deferred Claude Code work on 2026-09-09 until they have capacity; its native qualification is not a gate for the initial Mac Codex release. Preserve its existing adapter and regression coverage. Resume Claude Code integration and validation with Claude Code later, followed by Codex review. Windows follows Mac delivery, with Codex before Claude Code.
