...
- Keep macOS and Windows as design targets for both Codex Desktop and Claude Code. The maintainer set the order on 2026-09-13: start with Mac Codex, work on Windows Codex next, then resume Claude Code support. Claude native qualification is not a gate for either Codex delivery. Preserve its existing adapter and regression coverage; resume its integration and validation with Claude Code, followed by Codex review. Future targets must remain distinct from qualified support.
...
- The primary operating surface is the bundled local GUI opened inside Codex / Claude Code Desktop through the Unharness Skill (maintainer decision, 2026-09-14). The public site provides introduction, demos and installation/update guidance. Keep configuration, records and artwork local. GUI and local MCP use the same deterministic operations. Retain earlier public-operation receipts and bounded legacy compatibility during migration; do not promote new public pairings or make them a daily-use requirement.
