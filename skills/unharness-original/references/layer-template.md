...
拘束具・背景と旧本体の契約。新しい本体制作は[12コマの覚醒アニメーション](entity-motion.md)を使い、`entity-motion.png`を用意する。下記の本体1枚用の範囲は、新作の縮尺制限ではない。
...
下記は構造テスト用の要約で、取り込み用manifestではない。templateId・rendererVersion・固定アンカーは変えない。制作側で部品名を増やさない。拘束具の49姿勢の内容や追加の関節を推測しない。不一致を見つけたら親側のテンプレート更新として扱い、制作側で数字を変更して整合したことにしない。
...
拘束具・背景と旧本体の部品は左上を(0,0)とする共通キャンバスを使う。本体の基準点(362,330)と表示範囲(216,48,292,527)はデザインの収まりを判断する基準で、画像の切抜き寸法ではない。元PNGの透明余白を保つ。大きい画像の作画時は全座標を一律の比率で扱い、部品だけを別々に中央寄せしない。
...
3つの論理役割はentity／restraints／background。13画像全てを毎回作るという意味ではない。拘束具は6本のarm、4枚のpanel、glintの11部品に分かれる。個別に変更し、残りを保持できる。描画順は `background → rear-restraints → entity → front-restraints`。拘束具・背景と旧本体の保存PNGは724×724の同じ座標を保つ。新作の本体は3姿勢シートからプラグインが同じ倍率で正規化する。
...
新作の確認では、置換PNGと保持する部品を同じtemplateIdの姿勢0／24／48で合成する。背景の画像IDは同じで、本体v1は既存の画像と表示値を保つ。本体v2はモードごとの3姿勢と固定の覚醒演出を使う。拘束具の既存の動きを別の静止画3枚で置き換えない。Normalで覆われ、UNSEALで一部が開き、TRUEFORMで解放されることを目視する。各姿勢の接点・欠け・意図しない重なり・背景に埋もれる箇所を確認する。

既存の合成手段が使えなければ、制作コピーへのガイド重ね合わせは可能でも、正確な3モード合成は未検証とする。PNGを残してローカルGUIの検証を待つ。一枚絵しか生成できない場合も、分離できた実ファイルだけを成果とし、分離済みと装わない。

## 制作場所からの引き渡し

利用者が選んだ制作場所の新しい版に、置換用PNGだけをまとめる。`entity-motion.png`（新作の本体）や拘束具などのpartId対応名は案内のためで、ファイル名だけで自動保存を許可するものではない。元画像、任意の制作補助コード、3モードの見本は読込対象とは分離する。
...
このSkillは制作手順と参考情報のみ。同梱素材への参照を保つ配布レイアウト、manifestへの追加、GUIの明示的ファイル選択・レビュー・保存・コレクションは親側で統合する。現在のローカルMCPには制作場所の発行・確認、既知partIdからの読込レビュー、レビューの照会と保存がある。実際の導入版でtool一覧を確認して使い、存在しない操作を仮定しない。公開4操作へローカルの制作場所やパスを渡さない。

初回のMac Codex配布はagents/openai.yamlの明示呼出しポリシーを使う。Codexのプラグイン検査が拒否するdisable-model-invocation: trueは含めない。Claude Code向けの呼出し制御と配布は後続で確認する。アプリ内での表示名・呼出し名は実際の導入状態を確認する。別アプリへ勝手に会話を移したり、サブエージェントを作る手順はない。Codex／Claude Codeそれぞれの画像生成・プレビュー・読込・保存の実機一周は未確認。構造・契約テストは、AIの実際の挙動を証明するものではない。
