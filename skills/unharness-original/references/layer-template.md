...
拘束具・背景と旧本体の契約。新しい本体制作は[3姿勢と覚醒](entity-poses.md)を使い、`entity-poses.png`を用意する。下記の本体1枚用の範囲は、新作の縮尺制限ではない。
...
利用者が選んだ制作場所の新しい版に、置換用PNGだけをまとめる。`entity-poses.png`（新作の本体）や拘束具などのpartId対応名は案内のためで、ファイル名だけで自動保存を許可するものではない。元画像、任意の制作補助コード、3モードの見本は読込対象とは分離する。
