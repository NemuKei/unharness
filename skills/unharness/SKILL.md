...
- Keep existing memory, native continuity, execution permissions, managed/provider sources and required project conditions. This management Skill and its local MCP connection remain available in every mode. Never select the Unharness management plugin as a mode target. Other user-confirmed optional plugins use the separate registered whole-plugin control; never edit provider caches.
