...
User-facing favorite/checkpoint discovery uses bounded pages of at most 1,000 entries, with a lexical continuation cursor. Growing an immutable history must not disable checkpoint discovery. A bounded-all `listRecords` helper may reject larger buckets, but service listing and family validation use pagination. Cursors do not imply a snapshot across concurrent writes; IDs inserted before a returned cursor require a new listing. Scope filters operate within the selected global page and must retain its continuation cursor.
