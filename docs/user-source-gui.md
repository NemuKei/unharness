...
The primary pages are **モード・設定・外観**, with **比較・記録** and **接続・復旧** under **その他**. Initial registration is presented in Settings only after its absence is confirmed. **設定** → **保存した構成・相談方法を確認** → **保存した2構成を確認** reads a saved pair. V2 separates **零式から引き継ぐもの** from **限定解除で追加するもの**, with saved state counts and current eligibility. These are read-only lists; changes still require paired review. V1 remains an older rule. A source revision hides older loaded summaries. Conflicts, recovery and unknown state block changes but keep a **状態の確認をAIに頼む** copy action available. See [the workbench UX contract](spec-workbench-ux.md).

**未登録のSkillを確認する** opens a secondary enrollment flow. **追加候補の一覧を確認** reads the existing launch context. Choose one candidate and explicitly identify its origin, or copy the handoff to discuss candidates through MCP. The origin starts unconfirmed. V2 records roles only and explains that both modes are reviewed after registration; the legacy v1 form retains its per-mode invocation choices. **登録内容を確認** freezes a review; **この内容で登録** saves the expanded scope/Normal without changing configuration files. V2 then shows setup-required and preparation-required separately, disables new release plans and directs the user to the expanded-scope consultation. Saving the new pair does not prepare it. Normal and historical restore controls remain usable. Current-favorite capture and task observation remain unavailable while preparation-required. A lost enrollment response is not automatically resent; use **状態を再取得** to inspect the actual saved scope. Older favorites explain the added Skills' saved Normal treatment before restoration. See the [v2 browser evidence](evidence/2026-09-10-mode-inheritance-gui.md).
...
3. Choose Normal, **限定解除 — UNSEAL**, or **零式 — TRUEFORM**. For an adopted setup, selection changes only the displayed saved contents; **変更内容を確認** requests a plan. Legacy registrations retain their earlier selection/planning behavior. A saved setup controls release choices; **対象を調整** is available only under the earlier no-setup diagnostic contract. It cannot bypass v2 setup-required. UNSEAL shows the fixed guide when the saved pair selects it, with version/hash, review date and references. This is Unharness-authored material based on official guidance.
4. In **モード**, review the change summary next to the selection buttons, then click **この内容で確定する**. This applies only the stored plan. When it cannot run, its reason and next action appear in the same area. The prepared label follows service readback, not artwork. Global controls are shared by future tasks using this Codex home, including other projects, and remain prepared until restored. The base global AGENTS.md and repository AGENTS.md are preserved; the existing AGENTS.override.md operation switches selected global instructions. Start a fresh task yourself; the GUI never dispatches one.
5. In the development checkout, unrelated application-setting edits are checked as part of ordinary mode selection and kept when you confirm the mode. No separate Normal-version decision is needed. A mismatch is labelled **最後に準備したモード** until the confirmed plan succeeds. Changed instructions/Skills or an unprovable difference still stop the operation with a review path. Connection/recovery support also offers **変更を確認** → **共通設定だけを取り込む** for standalone reconciliation; that action keeps the displayed prepared mode and writes no managed file. Values remain private and older saved versions remain immutable. Released 0.0.10 and earlier workbenches retain the earlier separate review step until updated.
...
## Inspect saved mode contents

The development Mode page shows the selected saved instructions and Skills directly. Selecting an adopted mode previews its contents; **変更内容を確認** creates the change plan and **この内容で確定する** applies it. Viewing remains available through source conflicts. Instruction/Skill text is fetched only by an explicit read action, from that saved snapshot. The current prepared label is separate. Appearance preview is folded; records, settings, favorites and recovery remain under **その他**. See [the current screen contract](spec-workbench-ux.md).

## Review ordinary-use records

In the development workbench, open **その他 → 記録・比較 → 仕事を記録する**, then choose a job by name/date. The picker reads metadata for the registered project only; it does not collect all tasks' conversations. Selecting a job explicitly collects through its latest completed turn and excludes an ongoing turn. A job already recorded opens its saved record. **この仕事を記録する** in task-observation details selects that task. UUID entry and an explicit older cutoff remain under **別の方法で記録する**; the API's omitted cutoff still means the first turn for compatibility.

Choose whether the result was useful and optionally add a note about corrections or experience, then **この記録を保存**. The title is prefilled. Detailed criteria and assessor provenance are available in a disclosure. Saving opens the record; returning to the list keeps it available. One record is useful on its own. Select two or three records and choose **並べて見る** to see outcomes and notes before measurements. Unknown/partial evidence stays beside its value, and elapsed time is not presented as human effort. See [work-record UX](spec-work-record-ux.md). Published 0.0.10 and earlier builds retain the older manual-entry screen until updated.
...
Load paged history and open one record, or compare two or three unique saved versions. The detailed table and proportional bars use the service-returned order and recorded values, start from a common zero and keep exact numbers visible. `recordCount` counts selected versions, `distinctTaskCount` counts task UUIDs and `acceptedCount` counts versions satisfying the derived acceptance rule. Overlapping versions of one task, partial usage, overflow and zero accepted versions keep the service's null aggregate and reason; the browser does not invent another denominator. Every result remains observational and neutral; optional artwork is independent of the result.

**回答を開く** is the only comparison operation that returns the bounded saved answer. React renders it as escaped plain text with its record reference. **この記録の設定を保存** is available only for a frozen matched source association and records that exact historical snapshot/Normal/reference without applying it. In **モード**, use **保存版を表示** to load the saved favorites, then select one and review its restoration plan. The **モード** page and recovery controls stay reachable after comparison errors. Comparison draft, selection, history and opened output survive mode/revision changes in the same launch and scope, but clear when launch, context, workspace or registered scope changes. Delayed responses cannot reinstate superseded review/output/comparison intent; confirmed saves remain recorded.
...
In the saved start's details, choose **この条件で再実行**. The review uses the mode already prepared in **モード** and shows its frozen files and per-mode attempt budget. **この内容で再実行を準備** creates one owned work location. **開始状態を確認して依頼を受け取る** rechecks its files, configuration and source mapping before showing the exact saved request.
...
Use **再実行の履歴を読む** after reopening the browser or an uncertain operation. A confirmed result stays saved if the subsequent list refresh fails. No mutation is automatically retried. A stale task response is discarded; changed source state requires another handoff check. **この試行を取り消す** retains files and closes the attempt but does not stop a running Codex task. **モード** and Node-only source recovery remain available when replay data is unavailable.
...
| `mode-contents` | none | saved Normal and adopted per-mode instruction/Skill summaries, no bodies |
| `mode-source` | `mode`, `snapshotId`, `sourceId` | selected saved instruction/Skill body only |
...
| `recent-tasks` | optional `taskCursor` | bounded names/dates for the registered project; no transcript |
| `review-run` | `taskId`, optional `throughTurnId` or `latestCompleted: true` | private immutable review summary without output text |
