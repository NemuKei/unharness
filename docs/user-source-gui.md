...
After saving a release setup, **未登録のSkillを確認する** opens a secondary enrollment flow. **追加候補の一覧を確認** reads the existing launch context. Choose one candidate, explicitly identify its origin and select its UNSEAL/TRUEFORM invocation policy, or copy the handoff to discuss the candidates through MCP. The origin starts unconfirmed. **登録内容を確認** freezes a review; **この内容で登録** saves the expanded scope/Normal without changing configuration files. The workbench then shows the previous preparation and asks for a new mode plan. Current-favorite capture and task observation remain unavailable until that preparation. A lost enrollment response is not automatically resent; use **状態を再取得** to inspect the actual saved scope. Older favorites explain the added Skills' saved Normal treatment before restoration.
