...
After saving a release setup, **設定をAIに相談** → **保存した2構成を確認** reads the saved pair. V2 separates **零式から引き継ぐもの** from **限定解除で追加するもの**, with saved automatic/manual/disabled counts and a current eligibility disclosure. These are read-only lists; modifying either mode goes through the paired AI review. V1 is explicitly labeled as an older rule. Reading or copying changes no source files. Current source conflict, recovery or lost connection suspends the consultation. A source revision change hides an older loaded summary until it is read again.
