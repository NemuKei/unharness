...
5. Save the currently prepared configuration with an optional name. An empty name becomes `<preparedMode> · <revision>`, independently of the preview. **保存版を表示** and its cursor control retrieve immutable favorites. Selecting one creates a reviewed plan using its saved mode label.
6. **変更前への復帰を確認** plans the latest checkpoint; review and prepare it with the same action. **中断した変更を復旧** directly invokes guarded pending-journal recovery. Independent edits are retained or block restoration. Recovery remains visible on conflicts and uncertain responses.
...
Service-state and result fields follow [the registered-source contract](spec-user-sources.md). In particular `preparedMode` is Normal/UNSEAL/TRUEFORM even when plan `mode` is `favorite` or `checkpoint`; `conflict` is null or `{ kind }`; `recovery` contains `pending`, `lastCheckpointId` and `argv`. `verification` is nested and always leaves runtime/mode switching false, coverage unknown and nextTaskRequired true.
