...
[2026-09-11の配布候補検証](evidence/2026-09-11-final-native-candidate.md)では、この構造からの実際のCodex導入、同梱Nodeだけでの設定、Native MCP、アプリ内ブラウザーでの保存と復旧、ZIP展開後の一致を確認した。公開ZIPの実ダウンロードと内容照合は完了した。モデルによる導入相談、ダウンロード時の隔離属性付き起動、本物の公開HTTPSとの接続は引き続き未確認である。
