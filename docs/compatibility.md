...
The [persistent fixture runbook](desktop-observation.md) covers manual baseline → manual-only → fixed-only → restored baseline checks, explicit Skill selection, and fixture recovery. The [native Windows baseline](evidence/2026-09-07-windows-baseline.md) adds native filesystem recovery, hard-link publication and current-task recording projection, with 125 passing tests and one existing POSIX-FIFO skip. It does not observe a newly opened generated fixture task. The [fresh-task sequence](evidence/2026-09-06-desktop-fixture-macos.md) confirms baseline, manual-only omission, fixed-only AGENTS and refresh-assisted restoration on Mac, plus literal Skill invocation through a scoped file read. Two stale-catalog restore observations remain recorded. Configuration-based restart tests, desktop picker selection, complete source control and Windows fresh-task observations remain pending.

The [registered-loadout native smoke](evidence/2026-09-06-loadouts-macos.md) adds local filesystem/service evidence for immutable versions, source checkpoints and restoration on Mac. The [Windows baseline](evidence/2026-09-07-windows-baseline.md) independently repeats the local save/checkpoint/restore smoke. Neither local loop validates real personal sources; Windows still needs fresh favorite-associated tasks.
