...
- 今のモード（例：零式）と、「保存時と設定が違う」の表示の有無
- Codexの版（`codex --version`）。確認済みの版でないと、切替と取り込みは止まる
...
手順0〜3を実行し、手順4で止めた。このMacのCodexが `0.155.0-alpha.16.3` に更新されており、Unharnessは確認済みの `0.153.4` 以外ではCodex設定の読み書きを止めるため（計画のTask 6b）。手順6で0.0.11へ戻し、Codex設定が入替え前と同じであること、開発版のファイルが保存データに残っていないことを確かめた。手順0では確かめていなかったが、「保存時と設定が違う」の表示は入替え前から出ていた（設定が入替え前と同一に戻った後も同じ表示のため）。次回は手順0で、この表示の有無も控える。

## 2026-09-25の試行

`68af880` を組み立てて手順3まで進めた。Codex 0.155では設定の読み取りが通るようになったが、手順4の差分の確認が `source-redirection` で止まった。登録済みのSkill `~/.agents/skills/kanary` のフォルダーが、2026-09-24 13:17にKanaryアプリ自身の更新で入れ替わり（inodeが変わった）、零式のときにUnharnessが置いた `agents/openai.yaml` も消えていたため。独立した変更を上書きしないための正しい停止だが、今のUnharnessには、この入れ替わりを確認して登録し直す道がない（既存の `review-rebind` は古い記録方式の置き換えだけを扱う）。計画のTask 6cとして扱う。手順6で0.0.11へ戻した。Codexの設定は、導入元の項目の位置だけが変わり、TOMLとしての中身は入替え前と同じ。
