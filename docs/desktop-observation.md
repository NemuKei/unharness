...
The commands are portable structured Node operations, with no shell translation or paid API. Native Windows and WSL need separate observations. The [native Windows baseline](evidence/2026-09-07-windows-baseline.md) covers path handling, hard-link publication, lock/recovery behavior, and a selected current-task recording on Windows. The [first Windows fresh task](evidence/2026-09-07-windows-fresh-task.md) qualifies by its user-created route, cwd and timing, but its baseline markers do not match. It establishes neither desktop loading nor an explanation for the missing expected Skill catalog marker, and it does not establish Windows ACL isolation or named-pipe behavior; a passing Mac test is insufficient.

For the retained Windows fixture, the first exact-project READY task qualified but did not match baseline. The owner has refreshed the unchanged fixture Skill and reapplied the same baseline favorite to create a new receipt. Open another **new local task** through the same user-created route with unchanged model, reasoning and execution permissions, and collect its recording before changing the fixture. If that baseline matches, run manual-only (including explicit Skill invocation), fixed-only and restored baseline one new task at a time, preserving baseline at the end. Associate saved versions only through their corresponding application receipts, as described in [loadouts](loadouts.md#associate-a-fresh-task-with-the-saved-version).
