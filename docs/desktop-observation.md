...
The commands are portable structured Node operations, with no shell translation or paid API. Native Windows and WSL need separate observations. The [native Windows baseline](evidence/2026-09-07-windows-baseline.md) now covers path handling, hard-link publication, lock/recovery behavior, and a selected current-task recording on Windows. It does not cover a fresh generated-fixture task, and it does not establish Windows ACL isolation, named-pipe behavior, or desktop loading; a passing Mac test is insufficient.

For the retained Windows fixture, open the exact generated `project` folder as a **new local task** with the same model, reasoning, and execution permissions. Send only the fixture's marker-free READY prompt and collect that recording before changing the fixture. Then run the listed baseline → manual-only (including explicit Skill invocation) → fixed-only → restored baseline sequence one new task at a time, preserving baseline at the end. Associate the saved manual-only and baseline versions only through application receipts, as described in [loadouts](loadouts.md#associate-a-fresh-task-with-the-saved-version).
