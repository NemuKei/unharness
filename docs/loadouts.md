...
The [first Windows fresh-task observation](evidence/2026-09-07-windows-fresh-task.md) qualified for its current baseline application but was `not-matched-record`, so it is not a successful favorite association. The expected Skill catalog marker was absent; the Skill body was absent as expected. This does not identify a cause or alter the fixture configuration. The owner refreshed the unchanged Skill and reapplied the same baseline favorite to obtain a new receipt before the next same-route task.
...
Stores bind their canonical location. Moving/copying a store or scope requires future explicit migration/path mapping and is currently rejected. The [native Windows baseline](evidence/2026-09-07-windows-baseline.md) covers local registration, immutable saves, exact favorite/checkpoint restoration, path handling and hard-link publication. The first Windows fresh-task route completed but did not yield a matching favorite association. Windows ACL isolation, named-pipe behavior and desktop mode application remain unverified. Filesystem damage, missing ownership, power-loss durability and adversarial races retain the documented diagnostic limits. General personal-configuration recovery, full UNSEAL/TRUEFORM and MCP endpoints remain future work. The existing GUI remains limited to owned fixtures.
