...
For selection, the current recommendation is three candidates and one final adopted form, with technical retry handled separately from creative rerolls. The maintainer also raised one-shot and limited-remake alternatives, so this policy is not finalized. After adoption, offer a [build-card preview and X handoff](build-cards.md): save the card image and open editable post text in X; the initial flow still requires attaching that image there.
