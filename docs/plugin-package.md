...
The [archive checkpoint](evidence/2026-09-10-distribution-archive.md) additionally verifies ZIP extraction, complete identity preservation, launch permissions and supplemental dependency notices. That dated checkpoint was a private candidate; downloaded-file launch and the public client were qualified later, not by that check.

The [integrated candidate](evidence/2026-09-11-final-native-candidate.md) supersedes that assembly checkpoint for the v2 client/server and persistent Mac directory format. Its immutable payload is wrapped with a native local marketplace and Japanese installation request, then checked after ZIP extraction. The [wrapper recipe and templates](mac-installation.md) are separate from the plugin payload. That candidate report covers native installation, saved Normal/v2/artwork and same-distribution recovery. Its later public successors have the separate release evidence linked above.
