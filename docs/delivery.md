...
On 2026-09-24, the maintainer replaced the 2026-09-13 order with **Mac Codex redesign (0.1.0) → Mac Claude Code (0.2.0) → Windows**, so that both apps work on Mac first. The redesign follows the [chat-led redesign](superpowers/specs/2026-09-23-chat-led-redesign.md); heavy implementation and code review go to Codex (`gpt-6-sol`), design and UI supervision to Claude (Opus). Both Codex and Claude Code on macOS and Windows remain the final target. The earlier order (Mac Codex → Windows Codex → Claude Code) is kept below as history. The phase numbers below identify historical work packages, not numerical execution order. This priority does not establish support or authorize unqualified real-configuration writes.
