...
The [owned launcher](local-workbench.md) supplies `open_workbench` and `workbench_status`. The [plugin connection](plugin-connection.md) also starts before initial source registration, with a fixed locally configured context and a persistent request ledger; no tool can configure or register it. The remaining [domain/AI entry plan](superpowers/plans/2026-09-09-domain-workbench.md) adds the installable bundle and public-origin pairing through the retained management Skill. The [free layered appearance contract](personalization.md) supersedes performance-gated authoring as a product requirement; preserve existing operation receipts and data during migration.
