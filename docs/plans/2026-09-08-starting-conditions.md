...
- [x] Update availability docs together, check relative links and `git diff --check`, directly review and integrate the completed capture step. Integrated through `91c4c00`; the existing real workbench was rebuilt/restarted with source state and all six managed file entries unchanged. Normal revision 4 retained its pre-existing conflict. The full Mac goal stays active; continue into sequential replay and then AI/MCP.
