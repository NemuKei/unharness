...
Use separate immutable record roles for capture reviews, saved starts, manifests and binary chunks. A saved start references its exact review. Re-saving the same review returns the same identity and original date. A changed declaration needs a new review; old starts are never edited. Future attempts will reference a saved-start ID. Configuration favorites, ordinary comparison reviews and recovery state remain separate.
