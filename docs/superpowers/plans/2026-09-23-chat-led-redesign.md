...
- Create: `src/sources/preparation-history.mjs`（切替の履歴：追記と読み出し）
- Modify: `src/sources/transaction.mjs`（切替の成功と、復旧で状態が戻ったときに履歴へ追記）
...
2. タスクのモードは、そのタスクの開始時刻に準備されていたモードで決める。そのために、切替の成功（通常の適用と、復旧で状態が戻った場合の両方）のたびに `{ mode, revision, preparedAt }` をworkspace配下の追記専用の記録（切替の履歴）へ1行加える。既存の `state.json` や変更前の記録の形式は変えない。タスクの開始時刻以前で最も新しい履歴の行がそのタスクのモード。履歴の最初の行より前に始まったタスクと、判定できないタスクは集計しない（2026-09-24 Solの確認：既存の記録には過去の切替時刻が残らないため）。
...
**2026-09-24の追加（利用者が操作ごとの許可を選択）:** `0.155.0-alpha.16.3` の確認で、無効化・読み戻し・コメントと他の設定の保持は `0.153.4` と同じだったが、`enabled: true` へ戻すと明示的な項目が消えた（[記録](../../evidence/2026-09-24-codex-0.155-config-editor.md)）。通常装備への復帰・お気に入り・中断からの復旧は保存したバイト列を書き戻すため、この操作に依存しない。そこで版ごと・操作ごとに許可する。

5. 確認済みの一覧を「版 → 許可する操作」の形にする。`0.153.4` はすべて、`0.155.0-alpha.16.3` は `read`（設定の読み取り・差分の取り込み）と `disable`（`enabled: false` の書き込み）だけ。
6. `0.155.0-alpha.16.3` では、`enabled: true` の書き込み（限定解除で、通常装備でも無効だったSkillを有効に上げる）を行わず、`codex-version-unqualified` を返す。画面とAIは「このCodexの版では、このSkillを足すことはまだ確認していません。今の設定はそのままです」と出す。零式への切替、通常装備への復帰、お気に入り、差分の取り込みは使える。
7. 実機の実行ファイルを合成プロファイルで使い、零式への切替 → 通常装備への復帰が、元のバイト列に戻ることを確かめて、記録に追記する。

### Task 6c: 別のアプリが登録済みSkillのフォルダーを入れ替えた場合（Sol調査 → Opus設計確認 → Sol実装）

2026-09-25の実機確認で判明：Kanaryアプリが自身の更新で `~/.agents/skills/kanary` を入れ替え（inodeが変わり、Unharnessが零式で置いた `agents/openai.yaml` も消えた）、Unharnessのすべての操作が `source-redirection` で止まった。止まること自体は正しいが、利用者が先へ進む道がない。既存の `review-rebind` は古い記録方式（boot-local device番号）の置き換えだけを扱い、同じ場所の別inodeは対象外。

**Step 1（Sol・調査だけ）:** 次を調べて案を2つまで出す。
- 入れ替わったフォルダーを、通常装備・過去の版・お気に入りを書き換えずに「同じ登録対象の新しい世代」として記録し直せるか（既存の `registration-rebind` の世代の仕組みを参考にする）。
- 入れ替わったSkillの本文が変わっていた場合と、同じだった場合の扱い。本文が変わっていたら、利用者の確認を必須にする。
- 零式で置いていた設定（`agents/openai.yaml` など）が消えた場合に、それを「独立した変更」として保持したまま、零式を改めて準備する道。
- 1つの登録対象の入れ替わりで、ほかの登録対象の操作まで止まる現状を、その対象だけに閉じ込められるか。
...
**2026-09-25 実機3回目の追加:** 利用者の実環境では、kanaryの入れ替わりと同時に、Codexの共通設定（モデル・プラグインなど）にも2026-09-14以降の独立した変更があった。`review-replaced-source` は対象以外のファイルも保存時と同じことを求める（`assertCurrent`）ため `source-conflict` で止まり、共通設定を取り込む `plan-retained` は入れ替わりで止まる。お互いを待つ行き止まりになった。

9. 入れ替わりの確認と適用では、対象以外の差分のうち、`plan-retained` が扱える共通設定の差分（`config.toml` の保持対象）だけは妨げにしない。その差分は確認の記録に「あとで取り込む差分」として残し、書き換えない。それ以外の差分（別の登録対象の変更など）は今までどおり止める。
10. 入れ替わりの適用後は、共通設定の差分があれば既存の `plan-retained` → `accept-retained` で取り込め、その後に準備し直しへ進めること。画面の次の一手は、入れ替わりの確認 → 共通設定の取り込み（「Codexの設定がUnharnessの外で変わっています。変わった内容を確認して取り込みますか？」）→ 準備し直し、の順に1つずつ出す。
11. 差分があるときの「今のモード」は「確認中」ではなく準備済みのモード名を出し、「確認が必要です」を添える。

**テスト（合成workspaceで）:** 入れ替わりと共通設定の変更が同時にある状態から、確認 → 登録し直し → 共通設定の取り込み → 零式の準備し直し、が最後まで通る／別の登録対象も変わっているときは今までどおり止まる。

### Task 6d: 新しいCodexの版をこのMacで自動で確かめる（Sol、Opusが監修）

2026-09-25に判明：Codexは1日で `0.155.0-alpha.16.3` → `16.4` に自動更新された。版ごとに手で確かめて一覧に足す方法では追いつかない。利用者が案1（このMacで自動確認）を選んだ。

**Files（目安）:**
- Create: `src/codex/config-self-qualify.mjs`（合成プロファイルでの確認と、結果の記録）
- Modify: `src/codex/config-versions.mjs`（同梱の確認済み一覧に加え、このMacでの確認結果も参照する）
- Modify: 状態の読み取り・画面・管理Skill（確認中・確認後の表示）
- Test: `test/codex-config-self-qualify.test.mjs`（合成の実行ファイルで、通る場合・違いが出る場合・壊れた記録）

**振る舞い（テストで固定する）:**
1. 書き込みの前に、実行ファイルの版が同梱の一覧にも、このMacの確認記録にもない場合、自動確認を行う。確認は一時directoryの合成プロファイル（`HOME` と `CODEX_HOME` を向ける）と合成Skillだけを使い、利用者の設定には触らない。
2. 確認の中身は、2026-09-24にSolが手で行ったものと同じ：`read`（`config/read` の読み戻し、コメントと他の設定の保持）、`disable`（無効化と読み戻し）、保存したバイト列への復帰（零式 → 通常装備で元のバイト列）、`enable`（有効に戻したとき明示的な項目が残るか）、`plugin-disable`（同梱の既存fixtureと同じ観点。確かめられない場合は許可しない）。
3. 操作ごとに結果を記録する。記録には版、実行ファイルのSHA-256、確認日時、確認したUnharnessのsource revision、操作ごとの合否を入れる。記録はUnharnessのローカルデータ（workspaceの外、プラグインの保存データ側）に置き、改ざんや形式の崩れを検出したら使わずに確認をやり直す。
4. 実行ファイルのSHA-256が記録と違えば、同じ版番号でも確認し直す。
5. 通った操作だけを許可する。通らなかった操作は、今の `codex-version-unqualified` と文言のまま止める。
6. 確認中は画面に「新しいCodexの版を確認しています…」を出す。確認後は、使える場合は何も言わずに進み、一部が止まる場合は既存の文言を出す。確認は1つの版につき1回で、同時に2回走らない。
7. 確認に失敗した（合成プロファイルを作れない、時間切れなど）場合は、何も許可せず、書き込みも行わない。
