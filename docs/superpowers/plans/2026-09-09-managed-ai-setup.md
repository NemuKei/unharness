...
**2026-09-10更新:** モードの選択規則・提案形式・移行・継承表示は[公式プラグインと継承の専用計画](2026-09-10-official-plugin-mode-inheritance.md)を優先する。零式は選択した確認済み公式プラグインだけを任意の自動対象として残し、限定解除は全対象を継承して外部／自作Skillを追加する。本書の管理保護・Normal保持・初回導線は継続する。下記には当初の作成手順も残るため、すでに存在するファイルや検証済み処理を作り直さず、最新コードとの差分だけを実装する。新規則の合格は未取得。
...
**Spec:** [設定相談](../../spec-guided-setup.md)、[製品仕様](../../spec.md)、[統合計画](2026-09-09-mac-product-experience.md)。 モード選択の現行契約は[公式プラグインと継承](../../spec-mode-inheritance.md)。
...
- [ ] 自作・外部・不明の役割分類と、公式マーケットプレイス由来の証拠を別に保存する。零式の確認済みプラグイン選択と限定解除だけの追加選択から両モードを合成し、モデル・参照・内容の版と一緒に固定する。詳細な形式・移行・拒否テストは2026-09-10の専用計画に従う。
...
- [ ] 旧Normal・お気に入り・比較・復旧記録を保つ移行を追加する。旧TRUEFORMの保存版を、新しい「公式プラグイン限定と限定解除への継承」の規則へ無断変換しない。保存と適用を分け、過去の版に現在の零式を注入しない。
...
- [ ] 自作と確認した新規の単独Skillは、Normalでは確認済み設定へ追加し、UNSEALでは追加対象として相談し、TRUEFORMでは明示呼び出しにする。公式プラグインの選択・更新は別の由来確認と対象一覧の固定を通し、追加Skillを無断継承しない。
...
- [ ] 確認では「零式から引き継ぐもの」と「限定解除で追加するもの」を分ける。継承項目は限定解除側だけで外せず、零式変更時には両モードへの影響を示す。公式候補を全選択せず、空の選択と未確認の理由を表示する。
