...
2026-09-10に[ビルド済み画面と同梱Skill](../../evidence/2026-09-10-mode-inheritance-gui.md)を更新した。保存した引き継ぎ／追加の読み取り、v2役割のみの登録、両モードの再確認待ち、保存後の準備、旧Normalへの復帰を確認した。公式候補の非空表示は描画専用の合成応答であり、サービスや実機の由来資格ではない。初回の新規タスクは最初の応答完了後に観測する案内へ合わせた。実際のAI相談とプラグイン制御はタスク6に残る。
...
- [x] 「零式から引き継ぐもの」「限定解除で追加するもの」が分かれ、限定解除だけで継承項目を外せないこと、公式未確認を選べないことをブラウザーテストに追加する。
- [x] 初期相談で公式候補の全選択をしない。空の零式も許容する。利用者の由来確認と公式掲載の証拠を混同せず、自作・その他の外部Skillは限定解除の追加として相談する。
- [x] 通常の画面に大きな選択フォームを増やさず、詳細はAI相談、採用前は両モードの差分確認という既存の流れを使う。項目数は確認した実状態から出し、無効・未確認・未登録の範囲を隠さない。
- [x] 「次の新規タスク向け」「同じCodex環境で共有される変更」、旧規則のお気に入り、保存後まだ未適用の状態を表示する。コピー失敗、切断、古いタブ、狭い画面でも復帰操作を保持する。
- [x] `npm run check`、`npm run build`、`node --test test/web-setup.test.mjs test/web-enrollment.test.mjs`を実行し、ビルド済み画面の表示・キーボード操作を確認してコミットする。
