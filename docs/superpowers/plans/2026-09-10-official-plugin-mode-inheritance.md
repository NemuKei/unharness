...
- [x] 選択した実機版の公式資料と利用できる読み取りコマンドを確認し、実際の取得経路・フィールド・不足項目を証拠文書に残す。2026-09-10の[調査](../../evidence/2026-09-10-plugin-provenance-macos.md)で公開掲載と導入状態の別取得を確認した。完全な導入内容の照合とプラグインの自動使用だけを変える制御は未確認であり、資格は`unknown`のまま。公式候補の実装・新規則対応済みとは扱わない。
