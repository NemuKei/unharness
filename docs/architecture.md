...
Updated 2026-09-20. Daily operations use the bundled local GUI; the static public site supplies introduction, demos and install/update guidance. Bounded legacy public transport and operation receipts remain compatible. [Local entry](spec-local-entry.md), [work records](spec-work-record-ux.md) and [appearance](personalization.md) define current contracts; [status](status.md) keeps development verification separate from qualified package/site snapshots.

Recent task discovery belongs to the application adapter. `src/codex/recent-tasks.mjs` uses bounded, version-qualified native metadata reads for the registered project and projects names/dates only. Explicit selection invokes existing run review; core/diagnostic modules do not import GUI code. Metadata qualification does not qualify configuration writes or desktop-loaded modes.
...
**Retired 2026-09-24** (see the [chat-led redesign](superpowers/specs/2026-09-23-chat-led-redesign.md)): public-site operation was removed. The paragraph below records the former design for reading old receipts; the pairing, remote controller and public workbench no longer exist. The [public-page backend](domain-connection.md) was a separate route/controller around the registered-source controller. Pairing owns local approval and in-memory origin/launch/scope-bound tokens. Public policy owns exact request fields and response projections; the remote controller binds plans/applies to durable public-operation receipts in the workspace's existing AI ledger. Source operations share the local GUI queue. Connection expiry applies before execution starts, while source journals and result publication remain independent of browser transport. The common source controller and ledger keep their existing contracts. Recovery-only/fixture servers do not enable public pairing. Public UI and browser permission qualification remain subsequent work.
...
[Start-record validation](../src/experiments/start-records.mjs) binds immutable declarations, manifests and source guards to the registered scope. List projections omit requests/file bodies and validate metadata without rereading all chunks. Explicit details verify chunks and return the request plus a metadata-only table. The starting-conditions form was retired with replay on 2026-09-24; saved starts remain readable through the MCP start tools. Draft edits invalidate reviews, delayed results cannot replace newer input, and successful or uncertain publication is kept distinct from auxiliary read failure.
...
## Retired: sequential replay

Replay under another mode and its GUI were retired on 2026-09-24 in the [chat-led redesign](superpowers/specs/2026-09-23-chat-led-redesign.md). The replay services, variants and result collection that remain in `src/experiments/` are kept only where saved records, starting conditions or appearance evidence still read them. Existing replay records stay on disk unchanged and do not block state, switching or recovery. Comparisons use saved runs.
