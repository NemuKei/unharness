...
Use the approved three-state PNG as the actual source textures, with versioned source hashes and frame/light bounds. Preserve its armor shapes, thick cables and fine branching core. Constant idle float, source-sampled glow and particles keep the visible scene alive. Mesh displacement and adjacent-portrait blending follow one opening/lifting path; reverse transitions traverse the same path. A new selection retargets from the current intermediate pose without a snap or a reset through Normal. Effects off uses the original selected portrait with unchanged mesh coordinates; hidden tabs pause active visual time. Browser rendering and default playback make no model or CDN calls. Keep text, controls, numbers and evidence as HTML. Every control has a real action; explanatory empty states replace unavailable outcomes.
