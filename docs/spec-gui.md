...
Use bundled background, armor and core PNG layers with versioned source/mask metadata. Constant idle float, charge, glow and particles keep the visible scene alive. One release path opens the shell before lifting the core; reverse transitions lower it before closing. A new selection retargets from the current intermediate pose without a snap or a reset through Normal. Effects off settles to the selected pose; hidden tabs pause active visual time. Browser rendering and default playback make no model or CDN calls. Keep text, controls, numbers and evidence as HTML. Every control has a real action; explanatory empty states replace unavailable outcomes.
