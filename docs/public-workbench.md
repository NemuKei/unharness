...
The maintainer selected the existing `deltahelmlab-unharness` Cloudflare Pages project, separate from the main DeltaHelm Lab site. The first approved rollout and its 21-file comparison are historical evidence. The [0.0.9 workbench release](evidence/2026-09-14-workbench-release.md) records the current archive, 48-file comparison and public entry checks. The [0.0.8 artwork qualification](evidence/2026-09-13-awakening-motion.md) retains the earlier live appearance selection; the [0.0.6 qualification](evidence/2026-09-13-mac-codex-completion.md) retains the native mode/control baseline. A host-specific `disable_rum` Configuration Rule prevents the parent zone's analytics injection on this site while leaving the parent site's settings unchanged.
