...
For TRUEFORM, build `skills.config` changes with the native Codex editor in a freshly owned temporary CODEX_HOME containing a private copy of the original TOML. The only write method is `config/batchWrite`, the only edited key is `skills.config`, the target is that owned copy, and `reloadUserConfig` is false. Preserve every unselected entry and every other parsed configuration value. Read/write RPC transport remains separate from the existing read-only factory. Restore and recovery do not need Codex or YAML: their target bytes are already saved.
