...
- Memory, native continuity, execution permissions and unselected sources remain unchanged. Independently edited retained Codex settings can be recorded only through the explicit version-reconciliation flow below. Current tasks are not relabelled; successful preparation means next-task settings were prepared, with `runtimeStateVerified: false` and `modeSwitchingVerified: false`.
...
An independent edit remains a saved-snapshot mismatch until the retained-configuration proof succeeds. During ordinary mode selection, a configuration-only mismatch can be reviewed as part of a frozen mode plan. Confirming that plan preserves current common settings and prepares the requested mode in one guarded transaction; selection/refresh/reconnect never adopt them. Other source edits still require separate review. Explicit `plan-retained` / `accept-retained` remain available for standalone record-only reconciliation. Their summary contains fixed category labels and `managedFilesChanged: 0`, never keys, values or raw configuration. Both paths preserve the registered scope and immutable Normal history, clear current task observations on publication and never promote verification. See [retained settings](spec-retained-settings.md).
...
Before registration, show one setup action and source selection with optional-role confirmation. After registration, ordinary use is mode selection, one reviewed prepare action, save and undo. Each release mode has a closed **対象を調整** disclosure for registered target selection and the fixed guide. Required/unsupported items are explained there, not exposed as working toggles. Memory/native continuity are retained information. Common-setting changes are included in ordinary mode planning when proven safe; users need not understand Normal versioning. Connection/recovery support retains **変更を確認** and **共通設定だけを取り込む**, explicitly naming the prepared mode that stays unchanged. Cross-Normal favorite/checkpoint plans say **現在の共通設定を維持して準備** and explain that an ordinary Save creates a new favorite version.

Every operation binds to metadata accepted by the UI and the server's launch identity. Refresh metadata before actions; a changed launch/home/project/workspace is shown before registration or application, including after failed metadata retrieval. Clear cached preparation and retained-setting plans when launch/context, scope, revision or active Normal changes. Reuse operation IDs for a duplicate result and reject changed payloads. Do not automatically retry writes or retained-settings acceptance after uncertain transport failures. Preserve checkpoint/recovery access during failures.
...
3. A synthetic profile runs register → UNSEAL → TRUEFORM → Normal → save/favorite restore → checkpoint undo. Exact original bytes/absence return. A retained-only edit can be reviewed and recorded in each mode without changing managed bytes; later modes preserve it, while selected/non-config edits, stale plans, after-review edits, duplicate payload changes, directory redirection and interruption block or recover as specified.
