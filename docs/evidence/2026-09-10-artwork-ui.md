...
Automated paste could not use the page's OS clipboard image because Browser Use's virtual clipboard was empty. Computer Use separately refused control of the Codex app for safety reasons; no alternative native automation or security bypass was used. Command+V remains unverified.

The user's permission to verify attachment before posting also allowed the ordinary file-selection fallback. The saved PNG was rehashed against the identity above, selected through X's browser file chooser, and allowed to finish preparing. X then displayed the complete three-mode card in its composer with media edit/remove controls. Its actual screenshot was inspected. The composer was left with that attachment and the editable confirmation text; the Post action was never used. This verifies file attachment, not clipboard paste or public posting.
