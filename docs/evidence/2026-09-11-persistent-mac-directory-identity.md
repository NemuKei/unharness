...
This is controlled device-number-change evidence on the Mac, not an actual restart/remount test. Replay-location ownership records are outside this change. Windows source publication remains unqualified. Recovery under denied external networking, fresh native model tasks, the final public client/domain, and the release package are separate work. Recovering an old UUID-less personal registration requires a separately reviewed decision; the new format does not establish that registration's historical volume identity.
