...
A subsequent Pro static review found one timing gap: an approval rejected at expiry stayed pending even after fresh details confirmed expiry. A browser reproduction failed before the fix. Matching fresh `expired`/`unavailable` details now end the approval/cancellation wait, without extending a ticket or automatically issuing another one. All **four local-connection browser cases** passed afterward, including the existing lost-success-response case and explicit-only replacement issuance. No backend permission change was required.
