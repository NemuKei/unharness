...
The browser-downloaded ZIP was subsequently opened from Finder with the Mac Archive Utility. Its extracted Node, launcher and distribution manifest retained `com.apple.quarantine`. Without removing that attribute, the bundled Node returned `v24.20.0`, the launcher displayed CLI help with system directories only in `PATH`, and its 7,403-file plugin payload validated to the published distribution ID. This qualifies Finder extraction and AI/CLI launch with quarantine retained; it does not claim a double-click application installer.

The published source, ZIP, HTTPS-to-loopback connection, actual WebMCP calls, source round trip and public artwork/card saving are now verified within the environment above. Reboot/remount, fresh Desktop model tasks using the installed package, AI-led initial setup/creation and positive automatic-only control of a nonempty official-plugin selection remain unqualified. The maintainer's strict legacy registration was not migrated or restored by this synthetic test. The development preview is not the completed Mac product.
