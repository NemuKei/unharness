...
The internal registered service now supports read, initial discovery, explicitly requested later prepared discovery, owned-item selection and appearance recovery. A later discovery or selection supplies its reviewed `expectedStateId`; a stale request cannot overwrite a newer choice. Calling initial discovery again reads the saved appearance. The immutable lifecycle supports the three-candidate/final-adoption rules, but those creation operations are not exposed through the service until canonical eligibility is connected. [Mac core evidence](evidence/2026-09-09-appearance-lifecycle-macos.md) records the current checks; rendering and the user-facing journey remain separate work.
