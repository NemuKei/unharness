...
PixiJS 8.20.1 is declared as a pinned dependency with an npm lockfile for the planned GUI. The current CLI/core does not load it. Dependency installation does not establish browser rendering, GUI integration or OS support; those checks accompany the first GUI slice.
