...
- The maintainer supports personal quests → equipment comparisons → build cards, and proposed original artwork generated by the user's own AI. An optional generation skill and useful memory roles are captured in [the personalization proposal](personalization.md); these are unimplemented and their release scope remains open.
