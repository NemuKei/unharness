...
The review freezes an expanded registration, Normal, current snapshot and two release presets. The earlier Normal's managed content is copied exactly; only the newly confirmed Skills are appended. A prior retained-settings conflict must be resolved through its separate review. Adoption writes one active-registration pointer in the existing private state, under the existing profile lock and recovery journal. The original reservation, immutable registration and all earlier records remain unchanged. Adoption writes no configuration file and requires a subsequent reviewed mode preparation. New sources keep their current bytes until that preparation. The GUI labels the previous preparation; current-source observation, current-favorite capture and replay preparation are unavailable until a mode or reviewed historical favorite is prepared. Ordinary measurements may still be saved with no source association and the explicit preparation-required issue. The expanded snapshot also fences older writers that cannot validate the new target set.
