...
**Development checkout (unreleased):** Daily operation now uses the bundled local GUI. Its main screen shows three mode selectors and the saved instructions/Skill states; other tools are under More. The public site provides introduction, demos, installation and update guidance. Work records have a named-task picker and a short result/note form; one record opens on its own and two can be compared. The published 0.0.10 package remains a separate snapshot. The public site's local-opening guide was [deployed on 2026-09-21](docs/evidence/2026-09-21-ui-publication.md); this did not update installed plugins. See [local entry](docs/spec-local-entry.md) and [work-record UX](docs/spec-work-record-ux.md).
...
This is an unreleased extension, not an update to the installed 0.0.10 package.
