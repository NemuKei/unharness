...
**These are the 2026-09-10 targets; the complete behavior is not yet qualified.** Inherited-set calculation and [v2 storage, old-writer rejection and offline cancellation](docs/evidence/2026-09-10-mode-inheritance-storage.md) have synthetic checks. Official-plugin verification/control, v2 enrollment and the UI/MCP/native journey still need the [remaining implementation and checks](docs/superpowers/plans/2026-09-10-official-plugin-mode-inheritance.md). Existing v1 presets and favorites keep their recorded meaning. See [the selection and inheritance contract](docs/spec-mode-inheritance.md).
...
Original artwork is planned as reusable local parts for the AI entity, restraints and background. Create or revise a part or set with your own AI, import it locally and freely select earlier work. There is no performance unlock, fixed three-candidate choice or forced BAD treatment; performance is reported separately. Existing artwork and history are preserved, and choosing a look never changes the harness. See the [layered appearance contract](docs/personalization.md).
